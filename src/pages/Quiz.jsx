...
const allQuestions = [
  ...
  { id: 3, question: "What is the role of the Native Circle Stack in the Arc ecosystem?", options: ["It is a UI framework for building frontends", "It provides deeply integrated, scalable infrastructure for seamless dApp deployment", "It is exclusively a cross-chain bridge to Ethereum", "It is a hardware wallet"], correctAnswer: 1 },
  { id: 4, question: "Which consensus mechanism optimization is heavily utilized by high-throughput networks like Arc?", options: ["Proof of Work (PoW)", "Delegated Proof of Stake (DPoS) with Agent validators", "Proof of Capacity", "Proof of Burn"], correctAnswer: 1 },
  ...
  { id: 9, question: "What is the primary advantage of Arc's dynamic fee market?", options: ["Fees are always zero", "It adjusts base fees algorithmically based on network congestion while burning a portion", "It allows miners to charge whatever they want", "It requires users to bid blindly"], correctAnswer: 1 },
  ...
  { id: 13, question: "What guarantees finality in Arc's consensus layer?", options: ["Waiting for 6 block confirmations", "Byzantine Fault Tolerant (BFT) fast finality gadgets", "Probabilistic finality over 24 hours", "Finality is never guaranteed"], correctAnswer: 1 },
...
function Quiz() {
  ...
  useEffect(() => {
    ...
    let asked = JSON.parse(localStorage.getItem('arc_asked_questions') || '[]');
    
    // Filter out already asked questions
    let availableQuestions = allQuestions.filter(q => !asked.includes(q.id));
    
    // If we have less than 5 available, reset the asked list so they can repeat
    ...
    // Save the newly picked question IDs back to localStorage
    const newAsked = [...asked, ...selected.map(q => q.id)];
    localStorage.setItem('arc_asked_questions', JSON.stringify(newAsked));
  ...
  const handleAnswerClick = (index) => {
    ...
    if (index === questions[currentQuestion].correctAnswer) {
      setScore(score + 1);
      // Update score in localStorage for Dashboard
      const currentScore = parseInt(localStorage.getItem('arc_score') || '0');
      localStorage.setItem('arc_score', currentScore + 1);
    }
  };
  ...
  const handleNext = () => {
    if (currentQuestion < questions.length - 1) {
      ...
      const completed = parseInt(localStorage.getItem('arc_completed_lessons') || '0');
      localStorage.setItem('arc_completed_lessons', completed + 1);
  ...
  return (
    <div className="quiz-container">
      <h1 className="text-center mb-6">Knowledge Check</h1>
      
      <div className="progress-bar-container">
        <div className="progress-bar" style={{ width: `${progress}%` }}></div>
      </div>
      
      <p className="text-center mb-6" style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
        Question {currentQuestion + 1} of {questions.length}
      </p>

      <div className="mb-8">
        <h2 style={{ fontSize: '1.25rem', marginBottom: '24px' }}>{currentQ.question}</h2>
